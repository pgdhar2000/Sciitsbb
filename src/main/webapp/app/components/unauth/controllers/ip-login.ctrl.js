...
(function() {
    ...
    angular.
    module('bbApp').
    controller("ipLoginController",ipLoginController);

		function ipLoginController($scope, $state, $location, $cookies, $http, UrlService, SuccessStatusService, LoginSvc){
			...
			$scope.$on('$locationChangeStart', function(event, next, current){
				...
				if((backButtonLogin) && (current.indexOf("/ip-login") !== -1)){
				...
				if(response.status == "200"){
					...
					if(response.data.responseCode == "200"){
						...
						if(angular.equals(response.data.message, "User login is successful")){
							...
							if(response.data.responsePayload.roleId == 5){
								console.log("ip");
								$state.go("ip-dashboard");
							}
						}else{
							$state.go("ip-login");
