(function() {
    ...
    angular.
    module('bbApp').
    controller("raLoginController",raLoginController);

		function raLoginController($scope, $state, $location, $cookies, $http, UrlService, SuccessStatusService, LoginSvc){
			...
			$scope.$on('$locationChangeStart', function(event, next, current){
				...
				if((backButtonLogin) && (current.indexOf("/ra-login") !== -1)){
						...
						if(response.status == "200"){
							if(response.data.responseCode == "200"){
								...
								if(angular.equals(response.data.message, "User login is successful")){
									...
									if(response.data.responsePayload.roleId == 3){
										console.log("ra");
										$state.go("registered-agent-dashboard");
									}
								}else{
