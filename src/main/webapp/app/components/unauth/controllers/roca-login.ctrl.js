...
(function() {
    ...
    angular.
    module('bbApp').
    controller("rocaLoginController",rocaLoginController);

		function rocaLoginController($scope, $state, $location, $cookies, $http, UrlService, SuccessStatusService, LoginSvc){
			...
			$scope.$on('$locationChangeStart', function(event, next, current){
				...
				if((backButtonLogin) && (current.indexOf("/roca-login") !== -1)){
						...
						if(response.status == "200"){
							if(response.data.responseCode == "200"){
								...
								if(angular.equals(response.data.message, "User login is successful")){
									...
									if(response.data.responsePayload.roleId == 2){
										console.log("roca");
										$state.go("roca-dashboard");
									}
								}else{
